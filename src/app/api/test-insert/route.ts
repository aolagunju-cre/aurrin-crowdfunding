...
export async function GET() {
  return NextResponse.json({
    ...
    supabaseKeyLen: process.env.SUPABASE_SERVICE_ROLE_KEY ? process.env.SUPABASE_SERVICE_ROLE_KEY.length : 0,
  });
}
...
export async function POST(req: NextRequest) {
  ...
  if (!url || !key) return NextResponse.json({ error: 'Env not set', url: !!url, key: !!key }, { status: 500 });
